...
/// same spirit as `sde-ipc`'s own `call_sde`/`subscribe_sde` shorthands
/// hardcoding `"sde"`.
...
/// to find "our own" window among comphwde's `ListWindows` result without
/// the caller having to know/pass the id.
pub const SHELL_APP_ID: &str = "com.hackeros.hackermode";

/// Default timeout for a single request/response round-trip. Generous
/// relative to `sde-ipc`'s own 300ms liveness-check default because
...
/// shorthand for `sde_ipc::socket_path_for(EXTERN_NAME)`, same pattern as
/// `sde_ipc::sde_socket_path()`.
pub fn socket_path() -> std::path::PathBuf {
    sde_ipc::socket_path_for(EXTERN_NAME)
}

/// Sends one request to `comphwde --extern-hacker-mode` and waits for its
/// response - shorthand for `sde_ipc::call(EXTERN_NAME, ...)`, same
/// pattern as `sde_ipc::call_sde`.
...
/// `comphwde --extern-hacker-mode` - shorthand for
/// `sde_ipc::subscribe(EXTERN_NAME)`, same pattern as
/// `sde_ipc::subscribe_sde()`.
pub fn subscribe() -> Result<Subscription, SdeIpcError> {
    sde_ipc::subscribe(EXTERN_NAME)
}

/// Asks comphwde to end the `--extern-hacker-mode` session cleanly.
...
/// correctly-set `WAYLAND_DISPLAY` for *this* compositor instance.
...
///
/// There is no dedicated `WrapperModeEnter` call in comphwde's IPC
/// protocol - it doesn't need one. Comphwde already exposes exactly the
/// two primitives this needs: [`find_shell_window`] (`ListWindows`,
/// filtered by [`SHELL_APP_ID`]) and `MinimizeWindow`. Composing them
/// here, purely client-side, keeps comphwde itself completely unaware of
/// "Hacker Mode" as a concept - the same design comphwde already uses for
/// SDE, which has no "panel show/hide" IPC call either and instead
/// pins/unpins its panel with plain `PinSurface`.
...
mod tests {
    ...
    fn socket_path_is_hacker_mode_specific() {
        assert_eq!(socket_path(), sde_ipc::socket_path_for("hacker-mode"));
